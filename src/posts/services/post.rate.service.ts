...
export class PostRateService {
    ...
    public async create(postId: string, isUpVote: boolean, user: IUser): Promise<void>{
        ...
        if (await this.rateModel.isAlreadyRated(this.ENTITY_POST_RATE, postId, user._id)) {
            throw new BadRequestError('Comment already rated');
        }

        const rate = await this.rateModel.create(user._id, isUpVote, this.ENTITY_POST_RATE, postId);

        await post.updateOne({ $push: { rates: rate }, 'user':user._id, 'overallRate': isUpVote ? ++post.overallRate : --post.overallRate });
    }
}