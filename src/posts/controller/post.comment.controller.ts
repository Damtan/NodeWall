...
export class PostCommentController {
    ...
    public async updatePost(@Body({ validate: true }) commentDto :CommentDto, @CurrentUser() user: IUser, @Param("id") id: string): Promise<string>{
        ...
        return await this.postCommentService.create(commentDto, id).then((result: IComment) => {
    ...
    @UseBefore(ValidationIdMiddleware)
    public async getPostComments(@Param("id") id: string, @QueryParam("page") limit : number = 20) : Promise<PostEntity>{
        const commentsWithRates = await this.postModel.getPostCommentsAndRates(id, limit);

        return plainToClass(PostEntity, commentsWithRates.toObject(), { strategy: 'excludeAll' });
    }
}