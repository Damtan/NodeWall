...
export class PostController {
  ...
  @inject(SharedTypes.IPaginatorService)
  private paginatorService: IPaginatorService;
  @inject(PostModel) private postModel: PostModel;

  @Authorized()
  @Post("/posts")
  public async createPost(
    @Body({ validate: true }) postDto: PostDto,
  ...
  public async getPosts(
    @QueryParams() query: PostsQuery
  ): Promise<PaginateDto> {
    return this.paginatorService.paginate(
      PostEntity,
      this.postModel.getPostsQuery(query),
      query.page,
      query.limit
    );
  }
  ...
  public async getPost(@Param("id") id: string): Promise<PostEntity> {
    const postWithComments = await this.postModel.getPostWithCommentsAndRates(
      id,
      5
    );

    return plainToClass(PostEntity, postWithComments.toObject(), {
      strategy: "excludeAll",
    });
  }
}
